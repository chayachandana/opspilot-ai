...
End every response with a 'Recommended Actions' section.""",
    ...
    # Create run manually so we can handle tool calls ourselves
    run = client.runs.create(
        thread_id=thread.id,
        agent_id=agent.id
    )

    # Poll until done, handling tool calls manually
    import time
...
if __name__ == "__main__":
    run_opspilot("Why did sales drop this week?")